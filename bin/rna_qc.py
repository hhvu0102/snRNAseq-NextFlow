...
fig, axs = plt.subplots(nrows=2, ncols=4, figsize=(4*4, 2*4))
...
sns.histplot(x='pct_cellbender_removed', data=metrics[#(metrics.filter_rna_emptyDrops == True) &
                                                      ...
                                                      (metrics.pct_cellbender_removed > 10) &
                                                      (metrics.pct_cellbender_removed < 90) &
                                                      (np.isnan(metrics.pct_cellbender_removed) == False)], ax=ax, log_scale=True)
ax.axvline(THRESHOLD_FRACTION_CB_REMOVED*100, color='blue', ls='--', label='%ambient removed threshold Multi-otsu= {:,}'.format(THRESHOLD_FRACTION_CB_REMOVED*100))
ax.legend()
ax.set_xlabel('10% < % ambient removed < 90%')
